...
class _PopularFilmsSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final popularFilmsAsync = ref.watch(AppProviderItems.popularFilmsProvider);

    return popularFilmsAsync.when(
      loading: () => const Center(child: CircularProgressIndicator.adaptive()),
      error: (error, stack) => const Center(
        child: ErrorView(),
      ),
      data: (popularFilmsModel) => genreAsync.when(
        ...
        data: (genreModel) {
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              final popularFilm = popularFilmsModel.results?[index];
              return Padding(
